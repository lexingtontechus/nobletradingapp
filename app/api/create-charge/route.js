...
export async function POST(req: Request) {
  ...
  const successUrl = `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/portal?sub=${subscriptionId}&renewed=1`;

  const resp = await fetch(
    `${HELIO_API_BASE}/v1/charge/api-key?apiKey=${encodeURIComponent(HELIO_API_KEY)}`,
    {
      method: "POST",
      headers: {
  ...
  const chargeUrl: string = data.pageUrl;
  const chargeToken = chargeUrl.match(/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i)?.[1] ?? null;
