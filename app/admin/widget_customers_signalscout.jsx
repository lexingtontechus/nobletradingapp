...
export default async function WidgetCustomersSignalScout() {
  ...
  const { data: widget_pivot_count_signalscout } = await supabase
  ...
  const currentCustomersNew = widget_pivot_count_signalscout.reduce(
    (sum, item) => sum + (item.active ?? 0),
    0
  );
  const currentCustomersExpired = widget_pivot_count_signalscout.reduce(
    (sum, item) => sum + (item.expired ?? 0),
    0
  );
  const currentCustomersRenewed = widget_pivot_count_signalscout.reduce(
